...
function drawA() 
{
 ...
 var width = svgA.node().clientWidth;
  var height = svgA.node().clientWidth;
 // var width = 700, height  = 500;
  ...
  var svgA = d3.select("body").append("svgA").attr("width",width).attr("height",height)
      .style("background", "white");
  
  //input info for data
  var info = [
              
              {type:"carbs", amount:234},
              {type:"proteins", amount:2140},
              {type:"vitamin", amount:.61762} ,
              {type:"fats", amount:2874}, 
              ];
  ...
  //combine
  var putSections = svgA.append("g").attr("transform", "translate(250,250)")
                      .selectAll("path").data(data);
  
  //putSections.enter().append("path").attr("d",theSegment).attr("fill",
  //function(d){return addColor(d.data.amount)});
  putSections.enter().append("path").attr("d",theSegment).attr("fill", "#66C2A5", "#ABDDA4", "#E6F598");
  //make text appera
